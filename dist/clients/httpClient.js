...
export class HttpMcpClient {
    ...
    config;
    isConnected = false;
    useOAuth = false; // Only enable OAuth when explicitly requested
    oauthProvider; // Keep OAuth provider instance
    constructor(packageId, config) {
        this.packageId = packageId;
        this.config = config;
        this.client = new Client({ name: "mcp-gateway", version: "0.1.0" }, { capabilities: {} });
    }
    async initializeOAuthIfNeeded(forceOAuth = false) {
    ...
    async connect() {
        ...
        try {
            ...
            if (errorMessage.includes("Client ID mismatch")) {
                ...
                const authError = new Error(`OAuth tokens are invalid (Client ID mismatch). Tokens have been cleared.\n` +
            ...
            if (errorMessage.includes("401") || errorMessage.includes("Unauthorized")) {
                ...
                const authError = new Error(`Authentication required. Use 'authenticate(package_id: "${this.packageId}")' to sign in.`);
