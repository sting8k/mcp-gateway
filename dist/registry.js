 ...
 */
function expandEnvironmentVariables(env, packageId) {
    if (!env)
        return undefined;
    const expanded = {};
    const warnings = [];
...
export class PackageRegistry {
    ...
    static async fromConfigFiles(configPaths) {
    ...
    static validateConfig(packages) {
    ...
    static checkForPlaceholders(packages) {
    ...
    async createAndConnectClient(packageId, config) {
        let client;
        if (config.transport === "stdio") {
    ...
    async closeAll() {
        ...
        const closePromises = Array.from(this.clients.values()).map(client => client.close().catch(error => logger.error("Error closing client", {
            error: error instanceof Error ? error.message : String(error),
        })));
    ...
    async reconnectWithAuth(packageId) {
    ...
    async triggerAuthentication(packageId) {
        const config = this.getPackage(packageId, { include_disabled: true });
        if (!config) {
            throw new Error(`Package not found: ${packageId}`);
        }
